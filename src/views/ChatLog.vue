  ...
  <div class="container">
    ...
    <el-dialog v-model="deleteDialogVisible" title="删除消息" width="700px" :close-on-click-modal="false">
      <el-table :data="rawChatData" style="width: 100%" @selection-change="handleSelectionChange" ref="deleteTable"
        :row-key="row => row.cid" tooltip-effect="dark" border>
...
import { addChatByBatch } from '@/apis/api'
...
function deleteSelectedMessages() {
  ...
  // 过滤掉被选中的消息
  const selectedIds = new Set(selectedDeleteRows.value.map(item => item.cid))
  rawChatData.value = rawChatData.value.filter(item => !selectedIds.has(item.cid))
